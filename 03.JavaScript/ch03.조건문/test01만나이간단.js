...
function pp(birthday) {
    const year = new Date().getFullYear() - new Date(birthday).getFullYear() //현year-생일year = 만나이
    const m = (new Date().getMonth()+1) - (new Date(birthday).getMonth()+1) 
    //현month-생일month > 양수면생일지남 음수면 생일안지남 0이면 판단이 필요함 
    if (m < 0 ||   ( m===0  && new Date().getDate < new Date(birthday).getDate   )  )  {    
 //생일이 안지났으면 -1
    return year -1
}
return year
